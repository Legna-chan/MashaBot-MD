...
const handler = async (m, {conn, usedPrefix, command}) => {
  ...
  conn.reply(m.chat, `🍡 Debes esperar ${msToTime(time - new Date())} para usar #robxp de nuevo.`, m, m);
  return;
  }
  let who;
  ...
  if (!who) {
  conn.reply(m.chat, `*🪷 Debes mencionar a alguien para intentar robarle XP.`, m, m)
  return;
    };
  if (!(who in global.db.data.users)) { 
  conn.reply(m.chat, `🪷 El usuario no se encuentra en mi base de datos.`, m, m)
return;
  }
